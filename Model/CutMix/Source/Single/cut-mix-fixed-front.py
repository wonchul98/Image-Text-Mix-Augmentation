    ...
    for filename in os.listdir(folder_path):
        if filename.endswith('.jpg') or filename.endswith('.jpeg') or filename.endswith('.png'):
            ...
            for data_info in data_info_list:
                ...
                # 원본 템플릿 이미지를 복사하여 사용
                template_img = original_template_img.copy()
                
                white_labelled_template_img = make_white_label(template_img, width_weight, height_weight, x_location, y_location)
                mixed_img = cut_generator(white_labelled_template_img, img, width_weight, height_weight, x_location, y_location)
