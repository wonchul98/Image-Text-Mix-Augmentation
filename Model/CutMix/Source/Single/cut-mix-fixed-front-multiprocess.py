...
def process_image_batch(image_filenames, templates, location_infos, prompts, folder_path, output_folder, data_dict, shared_train_prompts, shared_test_prompts):
    ...
    for i, filename in enumerate(image_filenames):
        ...
        for t_idx in range(5):
            ...
            # First 4 to train, last 1 to test
            if t_idx < 4:
                shared_train_prompts.append(prompt_content)
            else:
                shared_test_prompts.append(prompt_content)

def process_in_parallel(folder_path, templates, location_infos, prompts, output_folder, data_dict, train_prompts, test_prompts, num_workers=12):
    image_filenames = [filename for filename in os.listdir(folder_path) if filename.endswith(('.jpg', '.jpeg', '.png'))]
    
    # Split the dataset into chunks
    chunk_size = ceil(len(image_filenames) / num_workers)
    image_chunks = [image_filenames[i:i + chunk_size] for i in range(0, len(image_filenames), chunk_size)]
    
    # Use a multiprocessing Manager to create shared lists
    with Manager() as manager:
        shared_train_prompts = manager.list()
        shared_test_prompts = manager.list()

        # Process each chunk in parallel using ProcessPoolExecutor
        with concurrent.futures.ProcessPoolExecutor(max_workers=num_workers) as executor:
            futures = [executor.submit(process_image_batch, chunk, templates, location_infos, prompts, folder_path, output_folder, data_dict, shared_train_prompts, shared_test_prompts)
                       for chunk in image_chunks]

            for future in concurrent.futures.as_completed(futures):
                future.result()  # Wait for all processes to complete

        train_prompts.extend(shared_train_prompts)
        test_prompts.extend(shared_test_prompts)

if __name__ == '__main__':
    print("Reading fixed_front.json...")
    ...
    print("Loading train and test prompts...")
    train_output_filename = os.path.join(output_prompt_folder, "train.json")
    test_output_filename = os.path.join(output_prompt_folder, "test.json")
    
    train_prompts = []
    if os.path.exists(train_output_filename):
        with open(train_output_filename, 'r', encoding='utf-8') as outfile:
            train_prompts = json.load(outfile)

    test_prompts = []
    if os.path.exists(test_output_filename):
        with open(test_output_filename, 'r', encoding='utf-8') as outfile:
            test_prompts = json.load(outfile)

    print("Processing images with 32 workers...")
    process_in_parallel(image_folder, templates, location_infos, prompts, output_folder, data_dict, train_prompts, test_prompts, num_workers=32)

    print(f"Generated train entries: {len(train_prompts)}")
    print(f"Generated test entries: {len(test_prompts)}")

    with open(train_output_filename, 'w', encoding='utf-8') as outfile:
        json.dump(train_prompts, outfile, ensure_ascii=False, indent=4)

    with open(test_output_filename, 'w', encoding='utf-8') as outfile:
        json.dump(test_prompts, outfile, ensure_ascii=False, indent=4)
