    ...
    for info in data_info:
      ...
      with open(data_file_path, 'r', encoding='utf-8') as df:
        ...
        process_template(location_data, output_folder, output_prompt_folder, template_folder,
                         ...
                         all_prompts, validations, count, random_indices)
...
def process_template(location_data, output_folder, output_prompt_folder, template_folder,
    image_folder, image_list, count_per_temp, data_dict, prompt_folder, all_prompts,
    validations, count, random_indices):
  ...
  for b in range(workers):
    ...
    if b == (workers - 1) :
      batches.append((start, count + count_per_temp))
    else:
      batches.append((start, end))
    start += batchSize

  with ProcessPoolExecutor(max_workers=workers) as executor:
    for (startIdx, endIdx) in batches:
      executor.submit(generate_images, startIdx, endIdx, image_list, image_folder, rectangle_width,
                     rectangle_height, skew_angle, template, x_pixel_location,
                     y_pixel_location, specific_output_folder, template_name,
                     data_dict, answer_format1, answer_format2, question,
                     validations, all_prompts, random_indices)


def generate_images(startIdx, endIdx, image_list, image_folder, rectangle_width,
    rectangle_height, skew_angle, template, x_pixel_location,
    y_pixel_location, specific_output_folder, template_name,
    data_dict, answer_format1, answer_format2, question, validations, all_prompts, random_indices):
  # start_time = time.time()
  # print(f"Starting at {start_time}", flush=True)
  for idx in range(startIdx, endIdx):
    ...
    output_image_name = os.path.join(specific_output_folder,
                                     f"{template_name}_{image_name.split('.')[0]}.jpg")

    # Remove the alpha channel before saving
    result_image_rgb = cv2.convertScaleAbs(result[:, :, :3])  # Ensure 8-bit depth
    cv2.imwrite(output_image_name, result_image_rgb, [cv2.IMWRITE_JPEG_QUALITY, 90])
    ...
    # 프롬프트 생성
    prompt_content = {
      "id": f"{template_name}_{image_name.split('.')[0]}",
    ...
    # end_time = time.time()
    # print(f"Finished at {end_time}", flush=True)
...
if __name__ == '__main__':
    read_json("fixed-skewed.json", 10000)