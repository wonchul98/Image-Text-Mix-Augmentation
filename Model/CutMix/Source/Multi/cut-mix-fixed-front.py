...
def make_white_label(img1, insert_image_infos):
    W, H = img1.size
    
    for data_info in insert_image_infos:
...
def process_all_images_in_folder(folder_path, template_img_path, output_folder, output_prompt_folder, data_dict, location_data, prompt_folder, all_prompts):
    template_img = Image.open(template_img_path).convert('RGB')

    if not os.path.exists(output_folder):
        os.makedirs(output_folder)

    white_labelled_template_img = make_white_label(template_img.copy(), location_data["insert_image_infos"])

    insert_image_numbers = location_data["insert_image_numbers"]
    ...
    for i in range(0, len(images), insert_image_numbers):
        ...
        mixed_img = cut_generator(white_labelled_template_img.copy(), img_list, location_data["insert_image_infos"])

        output_filename = f"{location_data['template_name']}_{i // insert_image_numbers + 1}_{location_data['task']}.png"
        ...
        prompt_content = {
            "id": f"{location_data['template_name']}_{i // insert_image_numbers + 1}_{location_data['task']}",
